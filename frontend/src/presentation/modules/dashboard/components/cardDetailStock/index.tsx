    ...
    <Wrapper>
      <Header>
        ...
        >
          <ViewCompareStocks>
            ...
            {isCompare && (
              <ViewAnimationPresence show={isCompare}>
                <Icon icon={faCodeCompare} />
                {stocksToCompare.map((item) => (
                  <StockItem item={item} key={item} />
                ))}
      ...
      <Content>
        <ViewInformationAndDate>
          <div>
            <ValueStock>
              {formatCurrency(stockDetail?.lastPrice || 0)}
            </ValueStock>
          ...
          {stocksToCompare.length > 0 ? (
            <ViewAnimationPresence show={stocksToCompare.length > 0}>
              <Container style={{ display: 'flex', flex: 1, gap: 10 }}>
                <TableContainer style={{ flex: 0.3 }}>
                  <TableStockCompare data={stocksCompared} />
                </TableContainer>
                <ChartContainer style={{ flex: 1 }}>
                  <ChartLineHistory
                    stockHistory={stockHistory}
                    loading={loadingStockHistory}
                  />
                </ChartContainer>
              </Container>
            </ViewAnimationPresence>
